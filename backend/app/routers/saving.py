...
from dependancies import get_saving_repo, get_user_repo, get_transaction_repo
...
async def  deposit(goal_id: int,
    ...
    user_id = current_user.id
    try:
        updated_goal = await deposit_to_target(goal_id, request.amount, user_id, saving_repo, user_repo, transaction_repo)
        return updated_goal
...
async def withdraw(goal_id: int,
    ...
    user_id = current_user.id
    try:
        withdraw_request = await withdraw_target(goal_id, amount, user_id, saving_repo, user_repo, transaction_repo)
